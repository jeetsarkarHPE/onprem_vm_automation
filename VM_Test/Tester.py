...
def main():
    ...
    parser.add_optional_arguments(cli.Argument.OVA_PATH, cli.Argument.DATACENTER_NAME,
                                  cli.Argument.RESOURCE_POOL, cli.Argument.DATASTORE_NAME, cli.Argument.CLUSTER_NAME, cli.Argument.ESX_NAME)
    ...
    if args.esx_name:
        esxi = get_esxi(si,datacenter,args.esx_name)
    else:
        print("None")
...
def get_esxi(si,datacenter,name):

    """
    Fetching the names of the esxi
    """
    view_manager = si.content.viewManager
